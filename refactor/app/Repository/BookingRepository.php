...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobs($user_id)
    {
        ...
        if ($cuser->is('customer')) {
            list($emergencyJobs, $normalJobs) = $this->getCustomerJobs($cuser);
            $usertype = 'customer';
        } elseif ($cuser->is('translator')) {
            list($emergencyJobs, $normalJobs) = $this->getTranslatorJobs($cuser);
            $usertype = 'translator';
        }
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        $cuser = User::find($user_id);
        if (!$cuser) {
            return []; // Consider appropriate handling
        }

        return $cuser->is('customer')
            ? $this->getCustomerJobsHistory($cuser, $request)
            : $this->getTranslatorJobsHistory($cuser, $request);
    }

    private function getCustomerJobsHistory($cuser, Request $request) {
        $query = $cuser->jobs()->with('translator'); // Assuming a relation between User and Job
        ...
        return $query->get()->map(function ($job) {
            // Transform the job object if needed
            return [
                'id' => $job->id,
                'translator' => $job->translator->name,
                // Other required fields
            ];
        });
    }

    private function getTranslatorJobsHistory($cuser, Request $request) {
        $query = $cuser->translatedJobs()->with('customer'); // Assuming a relation between User and Job for translators
    ...
    {
        $validationResult = $this->validateData($user, $data);
        if ($validationResult['status'] === 'fail') {
            return $validationResult;
        }

        return $user->is('customer') ? $this->storeCustomerJob($user, $data) : $this->storeTranslatorJob($user, $data);
    }
    ...
    public function storeCustomerJob(Request $request, User $customer)
    {
    ...
    public function storeTranslatorJob(Request $request, User $translator)
    {
        $validatedData = $this->validateData($request);

        $job = $translator->translatedJobs()->create([
            'title' => $validatedData['title'],
            'description' => $validatedData['description'],
            'status' => $validatedData['status'],
            // Other fields as required
